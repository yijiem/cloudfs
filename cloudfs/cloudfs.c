...
/*
 * acquire lock wrapper function for acquire rpc call
 */
void acquire_lock_wrapper(const char *path, struct fuse_file_info *fi) {
...
RETRANSMIT1:
	...
	lock_arg.operation = WRITE;

        write_log("acquire write lock of key:%s\n", lock_arg.key);
        retval = acquire_1(&lock_arg, &result_1, client);
	...
	write_log("acquire read lock of key:%s\n", lock_arg.key);
	retval = acquire_1(&lock_arg, &result_1, client);
	if (retval != RPC_SUCCESS) {
...
/*
 * release lock wrapper for release rpc call
 */
void release_lock_wrapper(const char *path) {
    enum clnt_stat retval;
    status result_1;
...
*/
  ...
  if (LOCK_SERVICE_ENABLE) {
      struct timeval tv;
      struct timeval tv2;

      client = clnt_create(SERVER, LOCKSERVICEPROG, LOCKSERVICEVERS, "udp");
      if (client == NULL) {
  	  write_log("Cannot connect to RPC server!\n");
...
static int cloudfs_release(const char *path, struct fuse_file_info *fi) {
    ...
    if (fi_fh->proxy) {
		...
		write_log("size smaller than %dB....delete file on cloud and metadata file....\n",
				state_.threshold);
		s3_delete(path);
		...
		goto SUCCESS;
                ...
                write_log("size still greater than %dB....flush to s3....\n", state_.threshold);
                s3_cloudfs_put(path);
	...
	// do not dirty or finish dealing dirty
        ...
        if (stbuf->st_size > state_.threshold) {
            write_log("file: %s size is %d greater than %dB\n	upload to cloud....\n", 
			absolute_path, stbuf->st_size, state_.threshold);
            s3_cloudfs_put(path);
            fd = open(metadata_path, O_RDWR | O_TRUNC | O_CREAT,
				S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
            if (fd < 0) {
		...
		goto FINISH;
            ...
            write_log("file: %s size is %d smaller than B\n	remain in SSD....\n",
			absolute_path, stbuf->st_size, state_.threshold);
        }
    }
SUCCESS:
...
FINISH:
    // unlock here
    if (LOCK_SERVICE_ENABLE) release_lock_wrapper(path);
...
static int cloudfs_getattr(const char *path, struct stat *stbuf) {
    ...
    // write_log("response from RPC server: %s", *resstring);
